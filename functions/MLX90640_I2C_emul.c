...
#define ROM_DATA_OFFSET         0x0000
#define RAM_DATA_OFFSET         0x0400
#define EEPROM_DATA_OFFSET      0x2400
#define REG_MLX_OFFSET          0x8000
#define REGISTERS_OFFSET        0x800D

#define STATUS_REG_ADDRESS      0x8000
#define CTRL_REG_1_ADDRESS      0x800D
#define CTRL_REG_2_ADDRESS      0x800E
#define I2C_CONF_REG_ADDRESS    0x800F
#define I2C_ADDR_REG_ADDRESS    0x8010
...
uint16_t * MLX90640_I2CInit(void)
{
    ...
    // load EEPROM memory
    memcpy(&Memory[EEPROM_DATA_OFFSET], EEPROM, EEPROM_LEN * sizeof(uint16_t));
    ...
    // This way the device is configured and prepared for operation.
    Memory[CTRL_REG_1_ADDRESS]   = Memory[EEPROM_DATA_OFFSET + 0x0C];
    Memory[CTRL_REG_2_ADDRESS]   = Memory[EEPROM_DATA_OFFSET + 0x0D];
    Memory[I2C_CONF_REG_ADDRESS] = Memory[EEPROM_DATA_OFFSET + 0x0E];
    Memory[I2C_ADDR_REG_ADDRESS] = Memory[EEPROM_DATA_OFFSET + 0x0F];

    emul_slave_addr = 0x33;//(uint8_t)(0x00FF & Memory[I2C_ADDR_REG_ADDRESS]);
    // assumes auto toggl of subpages
    emul_active_subpage = eSUBPAGE_0;

    // Memory[STATUS_REG_ADDRESS] &= ~(0x0010 | 0x0008 | 0x0001);
    Memory[STATUS_REG_ADDRESS] = 0x0000;
    Memory[STATUS_REG_ADDRESS] |= 0x0010;

    int readStatusRegCounter = 0;

    return &Memory[EEPROM_DATA_OFFSET];
}
...
int MLX90640_I2CRead(uint8_t slaveAddr, uint16_t startAddress, uint16_t nMemAddressRead, uint16_t *data)
{
    ...
    if(slaveAddr != emul_slave_addr)
    {
        return emul_slave_addr; 
    }

    memcpy(p, &Memory[startAddress], nMemAddressRead * sizeof(uint16_t));

    sensor_proces(startAddress, dummy, eRead);
...
int MLX90640_I2CWrite(uint8_t slaveAddr, uint16_t writeAddress, uint16_t data)
{
    ...
    memcpy(&Memory[writeAddress], &data, sizeof(uint16_t));

    sensor_proces(writeAddress, data, eWrite);
...
void sensor_proces(uint16_t addr, uint16_t value, i2c_operation_t operation)
{
    ...
    if(readStatusRegCounter > 10)
    {
        ...
        {
            // copy frame content to RAM
            if(emul_active_subpage == eSUBPAGE_0)
            {            
                copy_chess_pattern(Frame_0, eSUBPAGE_0);
                Memory[STATUS_REG_ADDRESS] |= (0x0001 & emul_active_subpage);
                // new active page
                emul_active_subpage = eSUBPAGE_1;
            }
            else if(emul_active_subpage == eSUBPAGE_1)
            {
                copy_chess_pattern(Frame_1, eSUBPAGE_1);
                Memory[STATUS_REG_ADDRESS] |= (0x0001 & emul_active_subpage);
                // new active page
                emul_active_subpage = eSUBPAGE_0;
            }
...
void copy_chess_pattern(const uint16_t *subframe, eSUBPAGE_t eSubpage)
{
    ...
    //     for(col = row % 2; col < 32; col += 2)
    //         Memory[RAM_DATA_OFFSET + row*32 + col] = subframe[RAM_DATA_OFFSET + row*32 + col];
    memcpy(&Memory[RAM_DATA_OFFSET], subframe, FRAME_LEN * sizeof(uint16_t));
}
